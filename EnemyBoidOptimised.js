class EnemyBoidOptimised {
    ...
    update(trail = false, update = true) {
        ...
        let poi_avoid_dx = 0;
        let poi_avoid_dy = 0;
        let friendly_boid_dx = 0;
        let friendly_boid_dy = 0;
        ...
        for (let poi of friendly_boids) {
            if (dist(this.position.x, this.position.y, poi.position.x, poi.position.y) < visible_range) {
                friendly_boid_dx += poi.position.x - this.position.x;
                friendly_boid_dy += poi.position.y - this.position.y;
            }
        }
        ...
        this.velocity.add(createVector(friendly_boid_dx, friendly_boid_dy).normalize().mult(enemy_boid_attack_constant));
